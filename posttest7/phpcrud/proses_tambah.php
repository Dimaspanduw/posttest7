  ...
  $nama_gambar_baru = $angka_acak.'-'.$gambar_produk; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
                move_uploaded_file($file_tmp, 'img/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO message (nama, email, notelp, alamat, gambar_produk) VALUES ('$nama', '$email', '$notelp', '$alamat', '$nama_gambar_baru')";
   ...
   $query = "INSERT INTO message (nama, email, notelp, alamat, gambar) VALUES ('$nama', '$email', '$notelp', '$alamat', null)";
                  $result = mysqli_query($koneksi, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($koneksi).
